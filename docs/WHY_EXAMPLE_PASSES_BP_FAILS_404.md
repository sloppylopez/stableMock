...
2. **Re-record with current ignore list**  
   Run **stableMockRecord** for `FullFlowFlexibleIT` with the **current** `@U(ignore = { ... })` so that new mappings are generated **with** placeholders for EchoToken, TimeStamp, TransactionIdentifier, UniqueID, ResID_Value, CorrelationID. Then run **stableMockPlayback** again. This addresses body-matching 404s **if** the request is already hitting the correct WireMock server.
